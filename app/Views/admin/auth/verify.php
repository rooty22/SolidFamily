...
<div class="auth-card" style="max-width: 480px; margin: 0 auto;">
    ...
    <form method="post" action="<?= url('admin/forgot-password/verify') ?>">
        ...
        <button type="submit" class="btn btn-primary w-100 py-3 font-bold shadow-md shadow-sky-500/20" onclick="document.getElementById('otp-code').value = Array.from(document.querySelectorAll('.otp-inputs input')).map(i => i.value).join('')">
            تأكيد الرمز
        </button>
    </form>
